...
def get_preferred_proxy_url():
  """Return the preferred single proxy URL for SDKs expecting one proxy value."""
  proxies = get_request_proxies() or {}
  return proxies.get('https') or proxies.get('http')


def apply_request_proxies_to_session(session):
  """Apply environment proxy settings to a requests Session if present."""
  ...
  try:
    ...
    # Keep session usable even if a custom session object doesn't expose proxies.
    return None
  return proxies
